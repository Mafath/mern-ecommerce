...
const Navbar = () => {
  ...
  const {user, logout} = useUserStore(); //initially user is null. 
    ...
    <header className='fixed top-0 left-0 w-full bg-gray-900 bg-opacity-90 backdrop-blur-md shadow-lg z-40 transition-all duration-300 border-b border-emerald-800'>
      <div className='container mx-auto px-4 py-3'>
        ...
        <nav className='flex flex-wrap items-center gap-4'>
          ...
          {/* check for the user and display the cart link */}
          {user && ( //if we have a user, we can use this cart link
            <Link to={'/cart'} className='relative group text-gray-300 hover:text-emerald-400 transition duration-300 ease-in-out'>
              ...
              {/* only if cart length is greater than 0, the number will be displayed */}
          ...
          {/* check for the admin and display the dashboard link */}
          {isAdmin && (
          ...
          {/* check whether user logged in or not and display logout or sign up/in buttons */}
          {user ?
          (
